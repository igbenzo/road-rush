...
document.addEventListener('keydown', (e) => {
  ...
  if (state === 'PLAYING') {
    ...
    if ((e.key === 'd' || e.key === 'D' || e.key === 'ArrowRight') && player.lane < LANE_COUNT - 1) {
      player.lane++;
      player.targetX = laneCenterX(player.lane) - CAR_WIDTH / 2;
    }
...
function spawnEnemy() {
  ...
  const colors = ['#e63946', '#4361ee', '#7209b7', '#2a9d8f', '#f77f00', '#e85d75'];
...
function drawGrass() {
  ...
  // simple grass texture stripes
  ctx.fillStyle = 'rgba(0,0,0,0.05)';
  for (let y = -40; y < canvas.height; y += 40) {
    const offset = (roadOffset % 40);
    ctx.fillRect(0, y + offset, canvas.width, 20);
  }
}
...
function drawRoad() {
  ...
  ctx.fillRect(ROAD_LEFT - 6, 0, 6, canvas.height);
  ctx.fillRect(ROAD_LEFT + ROAD_WIDTH, 0, 6, canvas.height);
}
...
function drawCar(x, y, bodyColor, isPlayer) {
  ...
  ctx.beginPath();
  ctx.ellipse(x + CAR_WIDTH / 2, y + CAR_HEIGHT - 4, CAR_WIDTH / 2, 8, 0, 0, Math.PI * 2);
  ctx.fill();

  // body
  ctx.fillStyle = bodyColor;
  roundRect(ctx, x, y, CAR_WIDTH, CAR_HEIGHT, 10);
  ctx.fill();

  // windshield / windows
  ctx.fillStyle = 'rgba(180, 220, 255, 0.75)';
  roundRect(ctx, x + 6, y + 10, CAR_WIDTH - 12, 16, 4);
  ctx.fill();
  roundRect(ctx, x + 6, y + CAR_HEIGHT - 28, CAR_WIDTH - 12, 16, 4);
  ctx.fill();

  // headlights or taillights
  ctx.fillStyle = isPlayer ? '#ffffff' : '#ffdd55';
  ctx.fillRect(x + 4, y + 2, 8, 5);
  ctx.fillRect(x + CAR_WIDTH - 12, y + 2, 8, 5);

  ctx.fillStyle = '#ff3333';
  ctx.fillRect(x + 4, y + CAR_HEIGHT - 7, 8, 5);
  ctx.fillRect(x + CAR_WIDTH - 12, y + CAR_HEIGHT - 7, 8, 5);

  // wheels
  ctx.fillStyle = '#111';
  ctx.fillRect(x - 3, y + 8, 5, 14);
  ctx.fillRect(x + CAR_WIDTH - 2, y + 8, 5, 14);
  ctx.fillRect(x - 3, y + CAR_HEIGHT - 22, 5, 14);
  ctx.fillRect(x + CAR_WIDTH - 2, y + CAR_HEIGHT - 22, 5, 14);
...
function drawPlayer() {
  ...
  drawCar(player.x, player.y, nitroActive ? '#00e5ff' : '#ffcc00', true);
}
